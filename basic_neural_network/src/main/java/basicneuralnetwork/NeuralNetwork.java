...
public class NeuralNetwork {
    ...
    public double[] guess(double[] input) {
        if (input.length != inputNodes){
            ...
            // chuyển 2D array sang simple matrix
    ...
    public void train(double[] inputArray, double[] targetArray) {
        if (inputArray.length != inputNodes) {
            ...
            for (int n = hiddenLayers + 1; n > 0; n--) {
                ...
                // Áp dụng delta to trọng số weight
                weights[n - 1] = weights[n - 1].plus(deltas);
                // tính toán và đặt lại target (sai số) cho layer trước
    ...
    private SimpleMatrix calculateGradient(SimpleMatrix layer,
                                           SimpleMatrix error, ActivationFunction activationFunction) {
        ...
        return gradient.scale(learningRate); // gradient * learning_rate
    }
